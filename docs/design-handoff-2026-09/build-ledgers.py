...
def add(path, source, task, treatment, target='', host='marketing'):
    ...
    rows.append(dict(path=path,host=host,source_template=source,packet=task,treatment=treatment,redirect_target=target,browser_observation=state,implementation_status='planned'))
for file in sorted(marketing.rglob('page.tsx')):
    ...
    if '[' not in route and route not in protected:
        ...
        elif route in ['/features','/how-it-works']: treatment='product'
...
with (out/'route-ledger.csv').open('w') as f:
    w=csv.DictWriter(f,fieldnames=list(rows[0]));w.writeheader();w.writerows(sorted(rows,key=lambda r:r['path']))
protected_rows=[]
for file in sorted(root.rglob('page.tsx')):
    ...
    protected_rows.append(dict(route_template=route,source=str(file),surface=surface,treatment='preserve; no redesign in this handoff'))
with (out/'protected-routes.csv').open('w') as f:
    w=csv.DictWriter(f,fieldnames=list(protected_rows[0]));w.writeheader();w.writerows(protected_rows)
