...
import { z } from "zod";
import { writeInviteResentEvent } from "./resend-invite-events";
...
export async function handleResendTeamInvite(invitationId: string) {
    ...
    const idParsed = idSchema.safeParse(invitationId);
    if (!idParsed.success) {
    ...
    if (!sendResult.sent) {
        ...
        if (orgId) {
            await writeInviteResentEvent(supabase, {
                orgId,
                businessId,
                userId: user.id,
                invitationId: row.id,
