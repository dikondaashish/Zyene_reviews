...
export async function handleProrationPreview(request: Request) {
  ...
  try {
    ...
    if (!r.success) {
      return apiError(r.error.issues[0]?.message || "Invalid body", {
        status: 400,
        details: requestId,
      });
    }
  ...
  try {
    activeMembership = await loadActiveBillingMember(user.id);
  } catch (error) {
    logger.error({ err: error, requestId }, "Billing membership lookup failed");
    return apiError("Could not verify billing access", {
      status: 500,
      details: requestId,
    });
  }
  ...
  try {
    ...
    if (subscriptionCustomerId !== org.stripe_customer_id) {
      ...
      return apiOk({ previewAvailable: false });
    }
    ...
    logger.error({ err: message }, "[proration-preview] Stripe createPreview failed:");
    Sentry.captureException(e, {
      tags: { route: "billing-proration-preview" },
    });
    return apiOk({ previewAvailable: false });
  }
}
