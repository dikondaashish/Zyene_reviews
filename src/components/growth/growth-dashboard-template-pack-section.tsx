...
function isEmptyReport(report: TemplatePackLeadReport): boolean {
    return (
        report.pageViews === 0 &&
...
export function GrowthDashboardTemplatePackSection({
    report,
}: {
    report: TemplatePackLeadReport;
}) {
    ...
    return (
        <section className="space-y-4" aria-labelledby="template-pack-lead-heading">
            <div>
                <h2 id="template-pack-lead-heading" className="text-lg font-semibold">
                    Template pack lead magnet
                </h2>
                <p className="text-sm text-muted-foreground mt-1">
                    Funnel for{" "}
                    <code className="text-xs bg-muted px-1 rounded">/resources/review-request-templates</code>
                    {" "}— last {report.periodDays} days (since {new Date(report.since).toLocaleDateString()}).
                    {report.excludesQaTraffic ? " QA traffic excluded." : null}
                </p>
            </div>
                    ...
                    {report.latestSubmissions.length > 0 ? (
                        <div className="overflow-x-auto rounded-lg border border-border">
                            <table className="w-full text-left text-sm min-w-[480px]">
                                ...
                                <tbody>
                                    {report.latestSubmissions.map((row) => (
                                        <tr key={`${row.subscribed_at}-${row.email}`} className="border-b border-border/60">
                            ...
                            <p className="text-xs text-muted-foreground px-3 py-2 border-t border-border">
                                Emails stored in DB; shown here for ops only (dashboard is auth-gated).
                            </p>
                        </div>
                    ) : null}
                </>
