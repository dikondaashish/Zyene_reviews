...
export async function loadBillingPageData(
  ...
  if (memberDataError) {
    logger.error(
      { err: memberDataError },
      "[Billing settings] Member fetch failed:",
    );
  ...
  const { data: orgRefreshed, error: orgRefreshError } = await admin
    .from("organizations")
    .select(
      "id, name, plan, stripe_customer_id, stripe_subscription_id, plan_status",
    )
    .eq("id", org.id)
    .single();
  ...
  if (orgRefreshError) {
    logger.error(
      { err: orgRefreshError },
      "[Billing settings] Organization refresh failed:",
    );
  ...
  const [emailRequests, smsRequests, linkRequests, smartReplies, businesses] =
    await Promise.all([
  ...
  const checkoutOffersTrial = await isEligibleForIntroTrial(
    orgLive.stripe_customer_id ?? null,
  );
