        ...
        <section className="space-y-4" aria-labelledby="visibility-heading">
            ...
            <div className="grid gap-4 xl:grid-cols-2">
                <Card><CardHeader><CardTitle className="text-base">Repeat-sampling variance</CardTitle></CardHeader><CardContent>
                    {data.variance.length ? <div className="space-y-3">{data.variance.slice(0, 8).map((row) => <div key={row.label}><div className="flex justify-between gap-3 text-sm"><span className="truncate">{row.label}</span><strong>{pct(row.rate)}</strong></div><p className="text-xs text-muted-foreground">{row.attempts} attempts · 95% Wilson interval {pct(row.low)}-{pct(row.high)}</p></div>)}</div> : <Empty text="Repeat a sampling run to measure variance." />}
                </CardContent></Card>
            ...
            <Card><CardHeader><CardTitle className="text-base">Mention sentiment, prominence, and attributes</CardTitle></CardHeader><CardContent>
                {data.mentions.length ? <div className="divide-y">{data.mentions.slice(0, 15).map((row, index) => <div key={`${row.brand}-${index}`} className="grid gap-1 py-2 text-sm md:grid-cols-[180px_100px_1fr]"><strong>{row.brand}</strong><span>{row.sentiment ?? "Not classified"}</span><span className="text-muted-foreground">{row.rationale ?? "Analysis unavailable"}{row.prominence !== null ? ` · prominence ${Math.round(row.prominence * 100)}%` : ""}{formatAttributes(row.attributes)}</span></div>)}</div> : <Empty text="Sentiment and attribute analysis will populate on new samples." />}
            </CardContent></Card>
        </section>
    );
}
