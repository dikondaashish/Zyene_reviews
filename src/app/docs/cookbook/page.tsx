                ...
                <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div id="doc-page-content" className="min-w-0 flex-1">
                        ...
                        <pre className="not-prose overflow-x-auto rounded-lg border border-border bg-muted p-3 text-xs">
                            <code className="font-mono text-muted-foreground">{`curl -X POST "${base}/api/v1/requests/send" \\
  -H "X-API-Key: zy_..." \\
  -H "Content-Type: application/json" \\
  -d '{"customerName":"Alex","customerPhone":"+18165551234","channel":"sms"}'`}</code>
                        </pre>

                        <h2 id="page-reviews">Page reviews</h2>
                        <pre className="not-prose overflow-x-auto rounded-lg border border-border bg-muted p-3 text-xs">
                            <code className="font-mono text-muted-foreground">{`curl "${base}/api/v1/reviews?page=2&limit=50" \\
  -H "X-API-Key: zy_..."`}</code>
                        </pre>

                        <h2 id="analytics">Analytics snapshot</h2>
                        <pre className="not-prose overflow-x-auto rounded-lg border border-border bg-muted p-3 text-xs">
                            <code className="font-mono text-muted-foreground">{`curl "${base}/api/v1/analytics?days=14" \\
  -H "X-API-Key: zy_..."`}</code>
                        </pre>
                    </div>
