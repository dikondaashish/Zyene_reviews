...
export async function GET(request: Request) {
    return runCronJob(request, { name: "sync-reviews", cadence: "twelve-hour" }, async ({ occurrenceKey }) => {
        try {
            ...
            if (error) {
                ...
                Sentry.captureException(error, {
                    tags: { route: "cron-sync-reviews", step: "fetch_platforms" },
                });
            ...
            if (platforms && platforms.length > 0) {
                try {
                    await inngest.send(
                        platforms.map((p) => ({
                            id: `cron:sync-reviews:${occurrenceKey}:${p.id}`,
                    ...
                    Sentry.captureException(inngestError, {
                        tags: { route: "cron-sync-reviews", step: "inngest_dispatch" },
                    });
                    await pingReviewSyncHeartbeat(false);
                    throw inngestError;
                }
            ...
            Sentry.captureException(error, {
                tags: { route: "cron-sync-reviews", step: "unexpected" },
            });
