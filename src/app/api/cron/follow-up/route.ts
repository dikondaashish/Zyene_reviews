 ...
 * Schedule externally every 15 minutes with `Authorization: Bearer CRON_SECRET`.
 *
...
export async function GET(request: Request) {
    return runCronJob(request, { name: "follow-up", cadence: "fifteen-minute" }, async ({ occurrenceKey }) => {
        try {
            ...
            if (!campaigns || campaigns.length === 0) {
                ...
                return NextResponse.json({
                    success: true,
                    processed: 0,
                    message: "No active follow-up campaigns found",
                });
            }
            ...
            await inngest.send(
                campaigns.map((c) => ({
                    id: `cron:follow-up:${occurrenceKey}:${c.id}`,
