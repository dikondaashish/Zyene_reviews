...
export async function GET(request: Request) {
    return runCronJob(request, { name: "aeo-alert-scheduler", cadence: "daily" }, async ({ occurrenceKey }) => {
        try {
            ...
            await inngest.send(
                businesses.map((b) => ({
                    id: `cron:aeo-alert-scheduler:${occurrenceKey}:${b.id}`,
